...
if(!empty($_POST["classid1"])) 
{
 ...
 while($row=$stmt->fetch(PDO::FETCH_ASSOC))
 {?>
  <p> <?php echo htmlentities($row['SubjectName']); ?><input type="number" id="marks" name="marks[]" min="0" max="25" value="" class="form-control" required="" placeholder="Enter marks out of 25" maxlength="2" autocomplete="off">
  <script>
  document.getElementById("marks").onkeyup=function(){
    var input=parseInt(this.value);
    if(input<0 || input>25)
    alert("Value should be between 0 - 25");
    return;
}    
</script>
  </p>
