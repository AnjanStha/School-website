...
if(!empty($_POST["classid1"])) 
{
 ...
 while($row=$stmt->fetch(PDO::FETCH_ASSOC))
 {?>
  <p> <?php echo htmlentities($row['SubjectName']); ?><input type="number" id="marks" name="marks2[]" min="0" max="100" value="" class="form-control" required="" placeholder="Enter marks out of 100" maxlength="3" autocomplete="off">
  <script>
  document.getElementById("marks").onkeyup=function(){
...
$id=$dta[0];
$id1=$dta[1];
 $query = $dbh->prepare("SELECT StudentId,ClassId FROM tblresult1 WHERE StudentId=:id1 and ClassId=:id ");
